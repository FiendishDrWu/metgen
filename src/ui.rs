use crate::config::{self, Config, Provider, Units};
        ...
        CentralPanel::default().show(ctx, |ui| {
            ui.with_layout(Layout::top_down(Align::Min), |ui| {
                ui.group(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("OpenWeather:");
                        ui.add(
                            TextEdit::singleline(&mut self.cfg.owm_api_key)
                                .hint_text("32-char key"),
                        );
                    });
                });
                ...
                Grid::new("inputs").striped(true).show(ui, |ui| {
                    ...
                    ui.label("Place name:");
                    ui.add(TextEdit::singleline(&mut self.place).hint_text("Missoula, MT"));
                    ui.end_row();
                    ui.label("Latitude:");
                    ui.add(TextEdit::singleline(&mut self.lat).hint_text("46.9163"));
                    ui.end_row();
                    ui.label("Longitude:");
                    ui.add(TextEdit::singleline(&mut self.lon).hint_text("-114.0906"));
                    ui.end_row();
                });
                        ...
                        thread::spawn(move || {
                            if !icao.is_empty() {
                                match fetch_noaa_metar(&icao) {
                                    Ok(Some(raw)) => {
                                        let _ = tx.send(UiMsg::Done(Ok(raw)));
                                        return;
                                    }
                                    Ok(None) => { /* fallthrough */ }
                                    Err(e) => {
                            ...
                            let icao_opt = if icao.is_empty() {
                                None
                            } else {
                                Some(icao.as_str())
                            };
                            let place_opt = if place.is_empty() {
                                None
                            } else {
                                Some(place.as_str())
                            };
                            let res: Result<WeatherSample, UiError> =
                                fetch_sample(icao_opt, place_opt, lat, lon, provider, &key);
                            match res {
                                Ok(s) => {
                                    let line = render_metar(&s, units, icao_opt);
                                    let _ = tx.send(UiMsg::Done(Ok(line)));
                                }
                                Err(e) => {
                                    let _ = tx.send(UiMsg::Done(Err(e.to_string())));
                                }
                            }
                        });
                    }
