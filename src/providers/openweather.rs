...
#[derive(Deserialize)]
struct OwmCurrent { dt: i64, coord: Coord, main: Main, wind: Option<Wind>, visibility: Option<f64>, weather: Vec<Wx>, clouds: Option<Clouds> }
#[derive(Deserialize)] struct Coord { lon: f64, lat: f64 }
#[derive(Deserialize)] struct Main { temp: f64, pressure: f64, humidity: f64 }
#[derive(Deserialize)] struct Wind { deg: Option<f64>, speed: Option<f64>, gust: Option<f64> }
#[derive(Deserialize)] struct Wx { id: i32 }
#[derive(Deserialize)] struct Clouds { all: Option<f64> }

#[derive(Deserialize)]
struct OwmOneCall { lat: f64, lon: f64, current: OcCurrent, hourly: Vec<OcHour> }
#[derive(Deserialize)] struct OcCurrent { dt: i64, temp: f64, pressure: f64, humidity: f64, dew_point: Option<f64>, wind_deg: Option<f64>, wind_speed: Option<f64>, wind_gust: Option<f64>, visibility: Option<f64>, weather: Vec<Wx>, clouds: Option<f64> }
#[derive(Deserialize)] struct OcHour { dt: i64, temp: f64, pressure: f64, humidity: f64, dew_point: Option<f64>, wind_deg: Option<f64>, wind_speed: Option<f64>, wind_gust: Option<f64>, visibility: Option<f64>, weather: Vec<Wx>, clouds: Option<f64> }

pub fn fetch_sample(icao: Option<&str>, place: Option<&str>, lat: Option<f64>, lon: Option<f64>, provider: Provider, api_key: &str) -> Result<WeatherSample, UiError> {
    let (lat, lon) = match (lat, lon, icao, place) {
        (Some(a), Some(b), _, _) => (a, b),
        (_, _, Some(code), _) if !code.is_empty() => lookup_icao_coords(code).ok_or_else(|| UiError::InvalidInput(format!("Unknown ICAO: {}", code)))?,
        (_, _, _, Some(p)) if !p.is_empty() => geocode_place(p, api_key)?,
        _ => return Err(UiError::InvalidInput("Provide ICAO, place name, or lat/lon".into()))
    };
...
fn geocode_place(q: &str, api_key: &str) -> Result<(f64,f64), UiError> {
    #[derive(Deserialize)] struct Hit { lat: f64, lon: f64 }
    let url = format!("https://api.openweathermap.org/geo/1.0/direct?q={}&limit=1&appid={}", urlencoding::encode(q), api_key);
    let client = Client::new();
    let res = client.get(url).send()?;
    if res.status().as_u16() == 401 { return Err(UiError::Unauthorized); }
    let arr: Vec<Hit> = res.json().map_err(|e| UiError::Parse(e.to_string()))?;
    arr.into_iter().next().map(|h| (h.lat, h.lon)).ok_or_else(|| UiError::NotFound)
}
...
fn fetch_current(lat: f64, lon: f64, api_key: &str) -> Result<WeatherSample, UiError> {
    ...
    Ok(WeatherSample {
        obs_time: chrono::DateTime::from_timestamp(m.dt, 0).unwrap().with_timezone(&Utc),
        lat: m.coord.lat, lon: m.coord.lon,
...
fn fetch_onecall(lat: f64, lon: f64, api_key: &str) -> Result<WeatherSample, UiError> {
    let url = format!("https://api.openweathermap.org/data/3.0/onecall?lat={}&lon={}&units=metric&appid={}", lat, lon, api_key);
    let client = Client::new();
    let res = client.get(url).send()?;
    if res.status().as_u16() == 401 { return Err(UiError::Unauthorized); }
    ...
    for h in m.hourly.into_iter().take(2) {
        hourly_next.push(HourlyPoint {
            dt: chrono::DateTime::from_timestamp(h.dt, 0).unwrap().with_timezone(&Utc),
            temp_c: Some(h.temp),
            dew_c: h.dew_point,
    ...
    Ok(WeatherSample {
        obs_time: chrono::DateTime::from_timestamp(m.current.dt, 0).unwrap().with_timezone(&Utc),
        lat: m.lat, lon: m.lon,
